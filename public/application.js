...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameUi = __webpack_require__(17);
	// new game setup
	var setup = __webpack_require__(13);
	// game id data
	var gameStore = __webpack_require__(16);
	// game logic
	var winCheck = __webpack_require__(20);
	...
	var onCreateGame = function onCreateGame(event) {
	  ...
	  //   })
	  // ).show()
	  gameApi.create().then(gameUi.createGameSuccess).catch(gameUi.createGameFailure);
	};
	// get current game data
	var onGetStats = function onGetStats(event) {
	  event.preventDefault();
	  // get data back from the server
	  gameApi.showGame().then(gameUi.showGameSuccess).catch(gameUi.showGameFailure);
	};
	// get all game data
	var onGetStatsAll = function onGetStatsAll(event) {
	  event.preventDefault();
	  // get data back from the server
	  gameApi.showAllGames().then(gameUi.showAllGameSuccess).catch(gameUi.showAllGameFailure);
	};
	// get complete games data
	var onGetStatsComplete = function onGetStatsComplete(event) {
	  event.preventDefault();
	  // get data back from the server
	  gameApi.indexGame().then(gameUi.indexGameSuccess).catch(gameUi.indexGameFailure);
	};
	...
	var onClick = function onClick(event) {
	  ...
	  if (setup.play % 2 === 0) {
	    ...
	    // send data to server
	    gameApi.updateGameState(data).then(gameUi.playSuccess).catch(gameUi.playFailure);
	    ...
	    // keep track of the box id
	    var _index = parseInt($(this).attr('id')) - 1;
	    ...
	    var _data = {};
	    // log the play in the appropriate spot for server
	    gameStore.game.cells[setup.play - 1] = _value;
	    // log the value in the order of the click
	    playOrderArray.push(_value);
	    // translate the data to the array format
	    setup.grid[Math.floor(_index / 3)][_index % 3] = arrayValue;
	    // call winCheck
	    winCheck.winCheck(setup.grid, setup.valArray);
	    // set game status equal to the right data
	    _data = { // should this be a constructor function to have one for each play?
	      'game': {
	        'cell': {
	          'index': _index,
	          'value': _value
	    ...
	    // send data to server
	    gameApi.updateGameState(_data).then(gameUi.playSuccess).catch(gameUi.playFailure);
	  }
	};
	...
	module.exports = {
	  ...
	  onGetStatsAll: onGetStatsAll,
	  onGetStatsComplete: onGetStatsComplete
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var config = __webpack_require__(6); // why the url in this file over another one?
	var store = __webpack_require__(11);
	var gameStore = __webpack_require__(16);
	...
	var updateGameState = function updateGameState(ajaxSend) {
	  return $.ajax({
	...
	var showAllGames = function showAllGames(ajaxResponseShowAll) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/',
	    method: 'GET',
	    headers: {
	    ...
	    data: ajaxResponseShowAll // actually don't need data here or in the function header
	  });
	};

	var showAllGames2 = function showAllGames2(ajaxResponseShowAll) {
	  return $.ajax({
	    url: config.apiOrigin + '/games/',
	    method: 'GET',
	    // headers: {
	...
	var indexGame = function indexGame(ajaxResponseIndex) {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	// new game setup
	var setup = __webpack_require__(13);
	var gameHistory = __webpack_require__(18);
	var ajaxReturn = __webpack_require__(19);
	...
	var createGameSuccess = function createGameSuccess(ajaxResponseCreate) {
	...
	var playSuccess = function playSuccess(ajaxResponsePlay) {
	  successes += 1;
	};

	var playFailure = function playFailure() {
	  errors += 1;
	};

	var showGameSuccess = function showGameSuccess(ajaxResponseShow) {
	  successes += 1;
	};

	var showGameFailure = function showGameFailure() {
	  errors += 1;
	};

	var showAllGameSuccess = function showAllGameSuccess(ajaxResponseShowAll) {
	  gameHistory.games = ajaxResponseShowAll.games;
	  gamesPlayed = gameHistory.games.length;
	  $('#games').text(gamesPlayed);
	  // run function
	  games = gameHistory.games.map(function (ajaxResponseShowAll) {
	    return ajaxResponseShowAll.cells;
	  });
	...
	var showAllGameFailure = function showAllGameFailure() {
	  errors += 1;
	};
	...
	var sendFailure = function sendFailure() {
	  errors += 1;
	};
	...
	module.exports = {
	  ...
	  showAllGameSuccess: showAllGameSuccess,
	  showAllGameFailure: showAllGameFailure,
	  // indexGameSuccess,
	  // indexGameFailure,
	  games: games,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var authUi = __webpack_require__(12);

	var winCheck = function winCheck(array, valArray) {
	  ...
	  for (var _i2 = 0; _i2 < setup.valArray.length; _i2++) {
	    if (setup.valArray[_i2] === 3) {
	  ...
	  // draw logic
	  if (gameStore.game.cells[8] === 'x' && setup.currentGame.winner === '' && setup.currentGame.loser === '') {
	    // && setup.play === 9
	    // change local games status
	    setup.currentGame.over = true;
